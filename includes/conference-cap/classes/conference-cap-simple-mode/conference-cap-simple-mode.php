...
class PWEConferenceCapSimpleMode extends PWEConferenceCap{
 ...
 public static function output($atts, $sessions, $conf_function, &$speakersDataMapping){
    ...
    foreach ($sessions as $key => $session) {
        ...
        $content .= '<div id="' . esc_attr($lectureId) . '" class="conference_cap_simple_mode__lecture-box">';

        $content .= '<div class="conference_cap_simple_mode__lecture-time-container">';
        $content .= '<h4 class="conference_cap_simple_mode__lecture-time">' . esc_html($time) . '</h4>';
        $content .= '</div>';

        $content .= '<div class="conference_cap_simple_mode__lecture-box-info">';
        $content .= '<h4 class="conference_cap_simple_mode__lecture-title">' . esc_html($title) . '</h4>';
