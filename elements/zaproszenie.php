...
class PWElementInvite extends PWElements {
     ...
     * @return object
     */
    private static function generate($html_to_pdf) {
        ...
        $pdf->WriteHTML($html_to_pdf, true, false, true, false, '');

        $pdf_url_path = plugin_dir_path(__FILE__) . '../pdf/zaproszenie.pdf';
    ...
    public static function output($atts) {
        ...
        if (isset($_POST['input1']) && isset($_FILES['input2'])) {
            ...
            self::generate($html_to_pdf);

            $output = '
            <script>
                window.location.href = "/wp-content/plugins/PWElements/pdf/zaproszenie.pdf";
            </script>';
            ...
            $output = '
                ...
                <h2> Wygeneruj zaprosznie na Wieczór Branżowy.</h2>
                <br>
                <form id="inv_form" method="post" action="" enctype="multipart/form-data">
                    ...
                    <button class="btn pwe-btn" name="submit" id="submit">Wygenerój</button>
                </form>
                </div>
            ';
